...
public class OpenCVFaceRecognizer {
    public static void main(String[] args) {
        ...
        File[] imageFiles = getTrainingFiles(trainingDir);

        MatVector images = new MatVector(imageFiles.length);
        Mat labels = new Mat(imageFiles.length, 1, CV_32SC1);
        prepareImages(imageFiles, images, labels);
    ...
    private static void prepareImages(File[] imageFiles, MatVector images, Mat labels) {
        IntBuffer labelsBuf = labels.createBuffer();

        int counter = 0;

        for (File image : imageFiles) {
            Mat img = imread(new BytePointer(image.getAbsolutePath()), CV_LOAD_IMAGE_GRAYSCALE);
    ...
    private static File[] getTrainingFiles(String trainingDir) {
